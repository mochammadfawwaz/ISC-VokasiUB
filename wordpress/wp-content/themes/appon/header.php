...
<?php
	$data_nicescroll_cursorcolor = ( ! empty( appon_global_var( 'scrollbar_color', 'color', true ) ) ) ? appon_global_var( 'scrollbar_color', 'color', true ) : '';
	$data_nicescroll_cursorwidth = ( ! empty( appon_global_var( 'scrollbar_width', 'width', true ) ) ) ? appon_global_var( 'scrollbar_width', 'width', true ) : '';
?>


<?php if ( ( 'default' != get_post_meta( get_the_id(), 'selectheader', true ) ) && ( get_post_meta( get_the_id(), 'selectheader', true ) ) ) { ?>
	<body <?php body_class(); ?> data-page-transition="<?php echo esc_attr( appon_global_var( 'page_transition_switch', '', false ) ); ?>" data-header-style="header-style-<?php echo esc_attr( get_post_meta( get_the_id(), 'selectheader', true ) ); ?>" data-nicescroll-cursorcolor="<?php echo esc_attr( $data_nicescroll_cursorcolor ); ?>" data-nicescroll-cursorwidth="<?php echo esc_attr( $data_nicescroll_cursorwidth ); ?>">
<?php } elseif ( ( appon_global_var( 'header-style', '', false ) ) ) { ?>
	<body <?php body_class(); ?> data-page-transition="<?php echo esc_attr( appon_global_var( 'page_transition_switch', '', false ) ); ?>" data-header-style="<?php echo esc_attr( appon_global_var( 'header-style', '', false ) ); ?>" data-nicescroll-cursorcolor="<?php echo esc_attr( $data_nicescroll_cursorcolor ); ?>" data-nicescroll-cursorwidth="<?php echo esc_attr( $data_nicescroll_cursorwidth ); ?>">
<?php } else { ?>
...
<?php wp_body_open(); ?>
	...
	<?php if ( appon_global_var( 'preloader_switch', '', false ) ) { ?>
		<!-- preloader -->
		<div class="preloader" data-preloader-timeout="<?php echo esc_attr( appon_global_var( 'preloader_timeout', '', false ) ); ?>">
			<div class="table">
				<div class="table-cell">
					<?php
					if ( ! empty( appon_global_var( 'preloader_style', '', false ) ) ) {
						include get_parent_theme_file_path( 'inc/preloader/' . appon_global_var( 'preloader_style', '', false ) . '.php' );
					}
					?>
				</div>
			</div>
	...
	<?php if ( appon_global_var( 'page_transition_switch', '', false ) ) { ?>
	...
	<!-- scrollup -->
	<?php if ( appon_global_var( 'scroll_to_top_switch', '', false ) ) { ?>
		<?php if ( ! empty( appon_global_var( 'scroll_to_top_direction', '', false ) ) ) : ?>
			<div class="scrollup <?php echo esc_attr( appon_global_var( 'scroll_to_top_direction', '', false ) ); ?>">
		<?php else : ?>
	...
	<?php if ( appon_global_var( 'gdpr_notice_switch', '', false ) ) { ?>
		<!-- gdpr-notice -->
		<div class="gdpr-notice alert alert-dismissible">
			...
			<div class="row">
				<div class="col-lg-9 col-md-9 col-sm-8 col-xs-12 text-left">
					<p><?php echo esc_html( appon_global_var( 'gdpr_notice_content', '', false ) ); ?></p>
				</div>
				<div class="col-lg-3 col-md-3 col-sm-4 col-xs-12 text-center">
					<a class="btn" href="<?php echo esc_url( appon_global_var( 'gdpr_notice_button_link', '', false ) ); ?>"><?php echo esc_html( appon_global_var( 'gdpr_notice_button_text', '', false ) ); ?></a>
				</div>
			</div>
			<!-- row -->
		</div>
	...
	<?php if ( class_exists( 'ReduxFrameworkPlugin' ) ) { ?>
		<?php if ( 'full-width' === appon_global_var( 'layout_type', '', false ) ) { ?>
			<div class="radiantthemes-website-layout full-width">
		<?php } elseif ( 'boxed' === appon_global_var( 'layout_type', '', false ) ) { ?>
	...
	<?php } ?>
		...
		if ( ! class_exists( 'ReduxFrameworkPlugin' ) ) {
			...
			if ( is_404() || is_search() ) {
				if ( ( appon_global_var( 'header-style', '', false ) ) ) {
					include get_parent_theme_file_path( 'inc/header/' . appon_global_var( 'header-style', '', false ) . '.php' );
				} else {
					include get_parent_theme_file_path( 'inc/header/header-style-default.php' );
				}
			} else {
				...
				} elseif ( ( appon_global_var( 'header-style', '', false ) ) ) {
					include get_parent_theme_file_path( 'inc/header/' . appon_global_var( 'header-style', '', false ) . '.php' );
				} else {
					include get_parent_theme_file_path( 'inc/header/header-style-default.php' );
				}
		...
		<div class="hamburger-menu-holder hidden">
			...
			<div class="hamburger-menu">
				...
				<div class="hamburger-menu-main">
					<?php dynamic_sidebar( 'appon-hamburger-sidebar' ); ?>
				</div>
				<!-- hamburger-menu-main -->
			</div>
			<!-- hamburger-menu -->
		...
		<!-- hamburger-menu-holder -->
			...
			<?php
			if ( class_exists( 'ReduxFrameworkPlugin' ) ) {
				if ( ( 'header-style-default' != appon_global_var( 'header-style', '', false ) ) ) {
					if (
